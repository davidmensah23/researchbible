...
import { Plus, LayoutGrid, Trash2, Share2, Globe, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
    ...
    const fetchProjects = useCallback(async () => {
        ...
        if (error) {
            ...
            // Map DB 'bibliography' -> 'references'
            const mappedProjects = data?.map(p => ({
                ...p,
                references: p.bibliography || []
            })) as Project[];
            ...
            <header className="h-20 bg-white border-b border-slate-100 px-10 flex items-center justify-between shrink-0 sticky top-0 z-50">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex flex-col items-end mr-4 hidden md:flex">
                        <span className="text-sm font-bold text-slate-900 leading-tight">Researcher</span>
                        <span className="text-[10px] font-medium text-slate-400">Scholar</span>
                    </div>
                                ...
                                <div className="flex-1 flex flex-col justify-between relative z-10">
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${project.status === 'Draft' ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-green-600'
                                                }`}>
                                                {project.status}
                                            </span>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xs font-bold text-slate-500">{new Date(project.updatedAt).toLocaleDateString()}</span>
                                            </div>
