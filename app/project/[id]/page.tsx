...
import {
    ...
    Languages, Info, Scissors, Share2, BarChart3, Users, Globe, FileDown, Table as TableIcon, Lock, Upload
} from 'lucide-react';
import LikeButton from '@/components/social/LikeButton';
import CommentsSection from '@/components/social/Comments';
...
export default function ProjectEditor() {
    ...
    const onUpdateSection = useCallback(async (section: ResearchSectionType, content: string) => {
        if (!project) return;
        ...
        const newSections = { ...project.sections, [section]: content };
    ...
    const onAddCitation = useCallback(async (paper: Paper) => {
        if (!project) return;
        if (project.references.find(ref => ref.id === paper.id)) return;

        const newRefs = [...project.references, paper];
        setProject(prev => {
            if (!prev) return null;
            return { ...prev, references: newRefs, updatedAt: new Date().toISOString() };
        });
    ...
    if (loading || !project) {
        return <div className="h-screen w-full flex items-center justify-center text-slate-400"><Loader2 className="w-8 h-8 animate-spin" /></div>;
    }
    ...
    return (
        <Dashboard
            ...
            onBack={onBack}
            setProject={setProject} // Pass setProject to update questions for Questionnaire
            supabase={supabase}
        />
    );
}

// --- Dashboard Component (Ported) ---
...
const Dashboard: React.FC<DashboardProps> = ({ project, onUpdateSection, onAddCitation, onBack, setProject, supabase }) => {
    ...
    // Persist questions when they change
    useEffect(() => {
        // Only update if questions differ from project
        if (project.questions !== questions) {
            // Optimization: Do not update DB on every keystroke, but we should eventually.
    ...
    const saveQuestions = async (newQuestions: Question[]) => {
        ...
        // Update DB
        setProject(prev => prev ? ({ ...prev, questions: newQuestions }) : null);
        await supabase.from('projects').update({ questions: newQuestions }).eq('id', project.id);
    }
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (result.success) {
                // Create a paper object from the uploaded file
                const paper: Paper = {
                    id: Math.random().toString(),
                    title: file.name.replace(/\.[^/.]+$/, ''), // Remove extension
                    authors: ['Uploaded Document'],
                    year: new Date().getFullYear(),
                    journal: 'Local Source',
                    abstract: result.extractedText.substring(0, 500) + '...'
                };

                onAddCitation(paper);

                // Also add to search results for immediate visibility
                const groundedSource: GroundedSource = {
                    title: paper.title,
                    authors: 'Uploaded Document',
        ...
        <div className="h-screen flex flex-col bg-[#F8F9FA] overflow-hidden">
                ...
                <div className="h-14 px-5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
                            ...
                            Saved: {new Date(project.updatedAt).toLocaleTimeString()}
                        </div>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm ${project.status === 'Published'
                                ? 'bg-green-100 text-green-700 border border-green-200'
                                : 'bg-slate-100 text-slate-600 border border-slate-200 hover:bg-slate-200'
                                }`}
                        >
                            {project.status === 'Published' ? <Globe className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
                            {project.status === 'Published' ? 'Public' : 'Private'}
                        </button>
            ...
            <div className="flex-1 flex overflow-hidden">
                        ...
                        <nav className="space-y-1 flex-1 overflow-y-auto no-scrollbar">
                            {sectionKeys.map((sec) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 text-xs font-bold transition-all rounded-2xl flex items-center gap-3 ${activeSection === sec ? 'bg-blue-50 text-blue-700 shadow-sm border border-blue-100' : 'text-slate-500 hover:bg-slate-50 border border-transparent'
                                        }`}
                                >
                ...
                <main ref={viewportRef} onScroll={calculateStats} className="flex-1 overflow-y-auto flex flex-col items-center bg-[#F1F3F4] relative pt-12 pb-48 scroll-smooth no-scrollbar">
                    ...
                    {activeSection === 'Questionnaire' ? (
                        ...
                        <div className="paginated-container group relative">
                            <div
                                ...
                                // Using data-placeholder via CSS
                    ...
                    {activeSection !== 'Questionnaire' && (
                        <div className="w-[816px] mt-24 mb-24 animate-in slide-in-from-bottom-10 print:hidden">
                ...
                <aside style={{ width: isSourceOpen ? '360px' : '0px' }} className="bg-white border-l border-slate-200 shrink-0 transition-all overflow-hidden print:hidden shadow-[-4px_0_25px_rgba(0,0,0,0.03)]">
                    <div className="w-[360px] flex flex-col h-full">
                        {activeSection === 'Questionnaire' ? (
                            ...
                            <AnalyticsPanel project={project} supabase={supabase} />
                        ) : (
                            ...
                            <div className="p-8 flex flex-col h-full animate-in slide-in-from-right-8 duration-700">
                                ...
                                <div className="mb-8">
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 text-white rounded-2xl text-xs font-bold hover:bg-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {isUploading ? (
                                            <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</>
                                        ) : (
                                            <><Upload className="w-4 h-4" /> Upload Source (PDF/DOCX)</>
                                        )}
                                    </button>
                                </div>
...
const AnalyticsPanel = ({ project, supabase }: { project: Project, supabase: any }) => {
    const [stats, setStats] = useState({ total: 0, today: 0 });
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        async function fetchStats() {
            const { count: total } = await supabase.from('responses').select('id', { count: 'exact', head: true }).eq('project_id', project.id);
            // In a real app we'd query by date for 'today', simplistic here
            const today = total;
        ...
        const channel = supabase.channel('responses')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'responses', filter: `project_id=eq.${project.id}` },
                (payload: any) => {
    ...
    const handleExport = async () => {
        const { data } = await supabase.from('responses').select('*').eq('project_id', project.id);
        if (!data || data.length === 0) return alert("No data to export.");

        // Flatten JSON data to CSV
        // Assuming all responses share the same question keys roughly
        const headers = ["Response ID", "Timestamp", ...Object.keys(data[0].data)];
        const csv = [
            headers.join(','),
            ...data.map((row: any) => {
                const values = [row.id, row.created_at, ...Object.values(row.data) as string[]];
        ...
        a.href = url;
        a.download = `survey-data-${project.id}.csv`;
        a.click();
    };
        ...
        <div className="p-8 flex flex-col h-full animate-in slide-in-from-right-8 duration-700">
            ...
            <div className="grid grid-cols-2 gap-4 mb-10">
                <div className="p-6 bg-slate-900 rounded-[24px] shadow-xl shadow-slate-200">
                    <span className="text-[9px] font-black text-slate-400 uppercase block mb-1 tracking-wider">Total Responses</span>
                ...
                <div className="p-6 bg-blue-50 rounded-[24px] border border-blue-100">
                    <span className="text-[9px] font-black text-blue-400 uppercase block mb-1 tracking-wider">Today</span>
                    <span className="text-2xl font-black text-blue-900">{loading ? '...' : stats.today}</span>
                </div>
            </div>
            ...
            <div className="pt-8 mt-auto border-t border-slate-100">
                <button
                    ...
                    className="w-full py-4 bg-slate-50 text-slate-900 rounded-[20px] text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center justify-center gap-3"
                >
                    <FileDown className="w-4 h-4" /> Export Raw Dataset (CSV)
                </button>
            </div>
        </div>
    );
